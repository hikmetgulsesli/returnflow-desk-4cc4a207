...
- ReturnFlow Desk turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser warehouse returns desk app called ReturnFlow Desk. It should help staff scan returned packages, triage return reasons, assign inspection lanes, track refund readiness, flag missing customer proof, manage exception notes, show empty and error recovery states, and every visible action s…
...
- Customer Operations - ReturnFlow Desk
- Customer Editor - ReturnFlow Desk
- Triage Board - ReturnFlow Desk
- Empty and Error Recovery - ReturnFlow Desk

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Customer Operations - ReturnFlow Desk
- surface_id: SURF_CUSTOMER_OPERATIONS
- unique_canvas_caption: Customer Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Customer data.
...
- data_entities: Customer, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Customer Editor - ReturnFlow Desk
- surface_id: SURF_CUSTOMER_EDITOR
- unique_canvas_caption: Customer Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Customer changes.
...
- data_entities: Customer, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_CUSTOMER_OPERATIONS -> Save returns to SURF_CUSTOMER_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Triage Board - ReturnFlow Desk
...
- purpose: Help users organize Customer work by triage priority, room readiness, consent, and handoff state when those signals are part of the requested product.
...
- entry_exit_rules: SURF_CUSTOMER_OPERATIONS -> Status changes keep the user in the same Customer context and expose recoverable failure feedback.
- design_guidance: Represent triage state clearly; do not introduce service-desk queues or unrelated operations modules.

SCREEN_SPEC_4:
- exact_screen_title: Empty and Error Recovery - ReturnFlow Desk
...
- purpose: Keep the Customer workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_CUSTOMER_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Customer workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_CUSTOMER_OPERATIONS - Customer Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Customer data.
   Data: Customer, ActivityEvent, Preference
...
2. SURF_CUSTOMER_EDITOR - Customer Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Customer changes.
   Data: Customer, ValidationError
   ...
   Entry/exit: SURF_CUSTOMER_OPERATIONS -> Save returns to SURF_CUSTOMER_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_TRIAGE_BOARD - Triage Board
   ...
   Entry/exit: SURF_CUSTOMER_OPERATIONS -> Status changes keep the user in the same Customer context and expose recoverable failure feedback.
...
4. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   ...
   Entry/exit: SURF_CUSTOMER_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Customer workflow and preserves unrelated state.
